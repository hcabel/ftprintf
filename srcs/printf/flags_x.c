...
/*   Updated: 2019/10/13 17:23:02 by sylewis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		flags_x(void *arg, t_flags flags)
{
	...
	if (flags.scale[0] == 'l')
		c = ft_utoa_base((unsigned long long)((unsigned long)arg), 16, 'a');
	else if (flags.scale[1] == 'h')
		c = ft_utoa_base((unsigned char)((long)arg), 16, 'a');
	else if (flags.scale[0] == 'h')
		c = ft_utoa_base((unsigned short)((long)arg), 16, 'a');
	else
		c = ft_utoa_base((unsigned long long)((unsigned int)arg), 16, 'a');
	nv.zero_size = 0;
	nv.space_size = 0;
	if (c[0] == '0' && flags.precis == 0)
		nv.arg_size = 0;
