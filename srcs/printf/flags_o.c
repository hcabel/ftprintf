...
/*   Updated: 2019/10/12 15:48:05 by hcabel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		flags_o(void *arg, t_flags flags)
{
	...
	if (flags.scale[0] == 'l')
		c = ft_itoa_base((unsigned long long)((unsigned long)arg), 8);
	else if (flags.scale[1] == 'h')
		c = ft_itoa_base((unsigned char)((long)arg), 8);
	else if (flags.scale[0] == 'h')
		c = ft_itoa_base((unsigned short)((long)arg), 8);
	else
		c = ft_itoa_base((unsigned long long)((unsigned int)arg), 8);
	nv.zero_size = 0;
	nv.space_size = 0;
